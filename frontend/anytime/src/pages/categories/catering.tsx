...
function InstantBookingBox({ category }: { category: string }) {
  ...
  return (
    <div className="rounded-2xl border bg-white p-4 shadow-sm">
      <div className="mb-2 text-[#8841FA] text-large font-extrabold">
        Instant booking
      </div>
...
const CATEGORY_META: Record<
  string,
  { title: string; icon: string; blurb: string; chips: string[] }
> = {
  catering: {
    title: "Catering",
    icon: "/service-icons/chef.svg",
    blurb:
      "Private chefs, event catering, meal prep & more — book vetted pros near you.",
    chips: [
      "Private Chef",
      "Buffet",
      "Cocktail Party",
...
export default function CategoryPage() {
  ...
  const results = useMemo(() => {
    ...
    let r = MOCK_RESULTS.filter(
      (s) =>
        (!q ||
          `${s.title} ${s.provider}`.toLowerCase().includes(q.toLowerCase())) &&
        s.rating >= minRating
    );
    r = r.sort((a, b) => {
    ...
    <div className="min-h-screen bg-[#F6F8FB]">
      ...
      <header className="sticky top-0 z-30 border-b bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center gap-4 px-4 py-4">
          <Link href="/services" className="text-[#6C38B8] font-semibold">
            ← Back
          </Link>
          <Image
            src={meta.icon}
  ...
  return (
    <div className="flex w-full items-stretch overflow-hidden rounded-2xl border bg-white shadow-sm">
      ...
      <Image
        ...
        width={240}
        height={180}
        className="h-auto w-60 object-cover md:h-full"
      />
      ...
      <div className="min-w-0 flex-1 p-5">
        <div className="flex items-center gap-3">
          ...
          {tag && (
            <span
              className={`rounded-lg px-3 py-1 text-sm font-bold text-white ${tagColor}`}
            >
              {tag}
            </span>
          )}
